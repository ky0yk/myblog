...
export class postRepository implements IPostRepository {
    ...
    async findById(id: PostId): Promise<Post | null> {
        const result = await this._prisma.post.findUnique({ where : { id: id.value } });
    ...
    async save(post: Post): Promise<void> {
        ...
        await this._prisma.post.upsert({
            where: {id: id.value},
            update: {authorId: authorId.value, title: title.value, content: content.value},
            create: {id: id.value, authorId: authorId.value, title: title.value, content: content.value}
        });
    }
