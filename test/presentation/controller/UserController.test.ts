...
describe('UserController', () => {
  describe('register', () => {
    it('should register a user and return status 201', async () => {
      mockRequest.body = { name: 'Alice', email: 'alice@example.com', password: 'myPassword' };
    ...
    it('should handle error when registering a user', async () => {
        ...
        expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Register error' });
      });
    });
  });
  ...
  describe('updateUser', () => {
    it('should update a user and return status 200', async () => {
      mockRequest.params = { userId: 'userId123' };
      mockRequest.body = { name: 'Bob', email: 'bob@example.com', password: 'newPassword' };
    ...
    it('should handle error when updating a user', async () => {
        ...
        expect(mockResponse.status).toHaveBeenCalledWith(400);
        expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Update user error' });
      });
    });
  ...
  describe('deleteUser', () => {
    ...
    it('should handle error when deleting a user', async () => {
        ...
        expect(mockResponse.status).toHaveBeenCalledWith(400);
        expect(mockResponse.json).toHaveBeenCalledWith({ error: 'Delete user error' });
      });
    });
