...
describe('AuthSerivice', () => {
    ...
    it('returns a token if login if successful', async () => {
        const user = new User(new UserId('userId'), new Name('John Doe'), new Email('john.doe@example.com'), new Password('hashedpassword'));
    ...
    it('throws error if JWT_SECRET_KEY is not defined', async () => {
        ...
        await expect(authService.login(new Email('john.doe@example.com'), new Password('hashedpassword'))).rejects.toThrow('JWT_SECRET_KEY is not defined.'); // Assert that an error is thrown
    });
});
